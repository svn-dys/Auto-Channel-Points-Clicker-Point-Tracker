    ...
    waitForElement("div[class*='community-points']").then((pointSection) => {
        const observer = new MutationObserver(mutations => {
            for (let mutation of mutations) {
                if (mutation.target.innerText.includes('+50') ||
                    ...
                    switch(mutation.target.innerText) {
                        case('+50'):
                            ...
                            console.log('Twitch Channel Points Collector: Value collected is 50. Adding points to total.');
                            return;
                        case('+60'):
                            addPointsToSyncProfile(60);
                            console.log('Twitch Channel Points Collector: Value collected is 60. Adding points to total.');
                            return;
                        case('+70'):
                            addPointsToSyncProfile(70);
                            console.log('Twitch Channel Points Collector: Value collected is 70. Adding points to total.');
                            return;
                        case('+100'):
                            addPointsToSyncProfile(100);
                            console.log('Twitch Channel Points Collector: Value collected is 100. Adding points to total.');
                            return;
                    }
                }
                return;
